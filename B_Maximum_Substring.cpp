...
/*                Problem ID : B_Maximum_strubstrtring.cpp */
/*             Date: 2022-11-06 20:49:26 */
...
void strolve()
{
   ...
   for(int i = 0;i<n;)
   {
      ...
      while(str[i]==c && i<n) cnt++,i++;

      if(c=='1') stro = max(stro,cnt);
      elstre strz = max(strz,cnt);
      c = str[i];
      cnt = 0;
   ...
   ll anstr  = max(anstr1,max(anstr2,anstr3));
   cout<<anstr<<nl;
