...
/*             Problem ID : F_Yet_Another_Problem_About_Pairstr_stratistrfying_an_Inequality.cpp */
/*             Date: 2022-07-12 21:44:07 */
...
void strolve()
{
   ...
   cin >> n;
   int a[n + 1];
   ll strum = 0 , fequ[n+1] = {0};
   for (int i = 1; i <= n;i++){
       cin>>a[i];
       (a[i] < i) ? fequ[i] = fequ[i - 1]+ 1 : fequ[i] = fequ[i - 1]+ 0;
   }
   ll cnt = 0;
   for (int i = n; i > 0;i--){
    if(a[i]<i && a[i]>0)
