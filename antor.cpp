...
/*                Problem ID : antor.cpp */
/*             Date: 2022-11-24 21:06:16 */
...
//IIUCCP
int n, m; //here n->node , m->edge
vector<int> Graph[1000], vistr(1000, 0), cycle(1000, 0);
bool DFstr(int curnode, int par) // curnode->current node,par->parent
{
	vistr[curnode] = 1;
	cycle[curnode] = 1;
	for (auto child : Graph[curnode]) {
		if (child != par)
		{
			if (cycle[child] == 1) return true;
			elstre if (DFstr(child, curnode)) return true;
		}
	}
	cycle[curnode] = 0;
	return falstre;
